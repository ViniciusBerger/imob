...
import { BadgeDollarSign, TrendingUp, TrendingDown, Wallet, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function FinancePage() {
    ...
    if (loading) return <div className="p-8 text-center text-gray-500">Carregando dados financeiros...</div>;

    // Calculations for progress bars
    const totalFlow = (stats?.totalRevenue || 0) + (stats?.totalFixedCosts || 0);
    const incomePercent = totalFlow > 0 ? (stats?.totalRevenue / totalFlow) * 100 : 0;
    const expensePercent = totalFlow > 0 ? (stats?.totalFixedCosts / totalFlow) * 100 : 0;
        ...
        <div className="space-y-8 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group">
                    ...
                    <p className="text-3xl font-bold text-gray-800">
                        R$ {Number(stats?.totalFixedCosts || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group">
                    ...
                    <p className={`text-3xl font-bold ${stats?.netIncome >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
                        R$ {Number(stats?.netIncome || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                    </p>
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                ...
                <div className="flex items-center gap-4 mb-2">
                    ...
                    <div className="flex-1 h-4 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-green-500 rounded-full" style={{ width: `${incomePercent}%` }}></div>
                    </div>
                    <span className="text-sm font-mono text-gray-600">{incomePercent.toFixed(1)}%</span>
                </div>
                <div className="flex items-center gap-4">
                    <span className="text-sm font-bold text-red-600 w-24">Despesas</span>
                    <div className="flex-1 h-4 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-red-500 rounded-full" style={{ width: `${expensePercent}%` }}></div>
                    </div>
                    <span className="text-sm font-mono text-gray-600">{expensePercent.toFixed(1)}%</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    <h3 className="text-lg font-bold text-gray-800 mb-4">Contas a Pagar (Financiamentos/Boletos)</h3>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                    ...
                    {/* Placeholder for Add Expense Button if needed */}
                </div>
                            ...
                            {expenses.length > 0 ? (
                                expenses.map((exp: any) => (
                                    <tr key={exp.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded text-xs font-bold uppercase">{exp.category || 'Geral'}</span>
                                        </td>
                                        <td className="p-4 text-gray-500 text-sm">{new Date(exp.date).toLocaleDateString()}</td>
                                        <td className="p-4 text-gray-500 text-sm">{exp.property?.address || '-'}</td>
